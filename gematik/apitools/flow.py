...
def check_url_exists(url: str) -> Optional[str]:
  response = http_session.get(url)
  if response.status_code == 200:
    return url
  else:
    print(f"URL dows not exist: {url}")
    return None
...
def flow():
    ...
    for node in flow.nodes:
      ...
      if node.type() == FlowVertexType.System:        
        ...
        row["url"] = check_url_exists(f"https://github.com/spilikin/api-tools/blob/main/docs/systems/{node.system}.adoc")
